...
const Support = () => {
  ...
  const [text, SetText] = useState<string>(t("Describe your issue"));
  ...
  return (
    <Wrapper>
      ...
      <Typography variant="body1" color='text.main'>
        {t('If you believe your account has been compromised, let us know and')} <br />
        {t('we can lock your account. This will disable items in your wallet from')} <br />
        {t('being bought, sold, or transferred using FastSwap.')} <Colortext>{t('Learn more.')}</Colortext>
      </Typography>
      <Space />
      <>
        ...
        <DownArea mode={isclick ? 'clicked' : 'notclicked'}>
          {listgroup.map((list, index) => (
            <MainContentLine key={index} onClick={() => { SetText(list); SetIsClick(false) }}>
              <BaseText>{t(`${list}`)}</BaseText>
            </MainContentLine>
          ))}
          <MainContentLine style={{ color: theme.palette.text.primary }}>{t('Other')}</MainContentLine>
        </DownArea>
      </>
